...
- `docs/slo.md` — three SLOs (processing latency, DLQ depth, circuit breaker open time) with explicit targets, the Prometheus recording rules computing them (`observability/prometheus/slo-rules.yml`), the chaos runs that validated each target, and the one new alert (`ProcessingLatencySLOBudgetBurn`) added alongside the pre-existing `DlqDepthHigh`/`CircuitBreakerOpenTooLong` alerts
