...
locust -f locustfile.py --headless -u 100 -r 10 --run-time 1m --host http://localhost:5000
```
...
Configuration notes:
- `--host` should point to the gateway base URL (default in examples: `http://localhost:5000`).
- The `locustfile.py` should POST to `/api/orders` with the `OrderPlaced` schema.
- Adjust user count (`-u`) and spawn rate (`-r`) to match your environment.

Verification:
- The gateway should respond with HTTP `202 Accepted` for valid `OrderPlaced` payloads.
...
Optional additions:
- Pin `locust` in `requirements.txt`.
- Add example environment variables or a config file for request payload customization.
