...
- **Given up:** `processed_events` grows without bound (one row per ever-processed event, never pruned) — acceptable at current scale but would need a retention/archival policy if event volume grows large enough for table size to matter.
