...
- **Add publisher confirms** to `RabbitMqPublisherWorker` (`ConfirmSelect` / `WaitForConfirmsOrDie` or the async equivalent) so a publish that the broker hasn't actually acknowledged doesn't get logged as success and doesn't return `202` to the client. **Resolved:** publisher confirms implemented in `RabbitMqPublisherAdapter` (`publisherConfirmationsEnabled: true`, `publisherConfirmationTrackingEnabled: true`); a nack or unconfirmed publish throws and triggers reconnect-with-backoff rather than logging a false success.
